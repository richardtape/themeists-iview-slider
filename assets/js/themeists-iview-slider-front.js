 ...
 */ 
 (function ($, window, undefined) {
	...
	var iView = function (el, options) {
			...
			if (iv.defs.total > 1 && ( iv.defs.timer == "360Bar" || iv.defs.timer == "Pie" ) ) {
				//Start the Raphael
			...
			$('.iview-video', iv.slider).click(function(e){
				...
				if(!$(e.target).hasClass('iview-video-close') && !$(e.target).hasClass('iview-caption') && !$(e.target).parents().hasClass('iview-caption')){
					video.show().animate({ top: 0 }, 1000, 'swing');
			...
			$('.iview-video-close', iv.slider).click(function(){
				...
				video.animate({ top: '-' + iv.defs.height + 'px' }, 1000, 'swing', function(){
					video.hide();
			...
			if (iv.options.controlNav) {
				...
				for (var i = 0; i < iv.defs.total; i++) {
					...
					if (iv.options.controlNavThumbs) {
						...
						var thumb = (slide.data('iview:thumbnail')) ? slide.data('iview:thumbnail') : slide.data('iview:image');
						iviewControl += '<a class="iview-control" rel="' + i + '">0' + (i + 1) + '. ' + slide.data('iview:title') + '</a>';
				...
				blocks.each(function (i) {
					...
					setTimeout(function () {
						if (fx == 'zigzag-grow-top' || fx == 'zigzag-grow-bottom') block.animate({
							...
							opacity: '1'
						}, (iv.options.animationSpeed * 2), 'swing', function () {
							if (i == totalBlocks - 1) iv.transitionEnd(iv);
						});
					}, (100 + timeDelay));
					timeDelay += 20;
